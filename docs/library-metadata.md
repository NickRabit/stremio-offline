...
- Artwork: `ArtworkQueue` is one-at-a-time. Local `poster.jpg` / `folder.jpg` (`POSTER_NAMES` in `server/src/artwork.ts`) always win. Missing art: catalog poster if `libraryMeta` exists, else ffmpeg thumbnail, written next to the media only where the library's `writeArtwork` allows it and to the hashed data-dir file otherwise (see [multi-library.md](multi-library.md) §8). Browse schedules this as a side effect of `GET /api/library/browse`.
