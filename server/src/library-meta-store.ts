...
export class LibraryMetaStore {
  ...
  constructor(dataDir = process.env.DATA_DIR ?? "/data") {
    this.dir = path.join(dataDir, "library");
  }

  async load() {
    await mkdir(this.dir, { recursive: true });
    const names = (await readdir(this.dir).catch(() => [])).filter((name) => name.endsWith(".json"));
    for (const name of names) {
      const raw = await readFile(path.join(this.dir, name), "utf8").catch(() => undefined);
  ...
  async update(libraryId: string, mutator: (file: LibraryMetaFile, episodes: Record<string, LibraryEpisodeRecord>) => void) {
    if (!libraryId) return;
  ...
  private fileFor(name: string) {
    return path.join(this.dir, name === "episodes" ? "episodes.json" : `${name}.json`);
  }
