...
export function App() {
  ...
  const trailerAction = (trailer: Trailer | null) => {
    if (!trailer) return null;
    if (settings.secureMode) return <a className="trailer-action" href={`https://www.youtube.com/watch?v=${encodeURIComponent(trailer.youtubeId)}`} target="_blank" rel="noopener noreferrer" title={t("trailers.openHint")}>{t("trailers.openOnYouTube")}</a>;
    return <button className="trailer-action" title={t("trailers.openHint")} onClick={() => setTrailerOpen(trailer)}>{t("trailers.watch")}</button>;
  };
              ...
              {browse.items.map((item) => item.kind === "library"
                ...
                ? <article className={`browse-item folder${browseFocus === item.path ? " focused" : ""}${selectedPaths.has(item.path) ? " selected" : ""}`} key={item.path} data-path={item.path} aria-current={browseFocus === item.path ? "true" : undefined}><button className="library-open" onClick={() => { if (selectionMode) { toggleSelection(item.path); return; } setBrowseQuery(""); setFromFavorites(browsePath === ":favorites" || fromFavorites); setBrowsePath(item.path); }}>
                    ...
                    {!selectionMode && menuFor === item.path && <span className="browse-actions" onClick={(event) => event.stopPropagation()}>
                      {item.match === "matched" && <>{trailerAction(libraryTrailers[item.path] ?? null)}{titleLinksRow(libraryLinks[item.path] ?? [])}</>}
                      {matchActions(item)}
