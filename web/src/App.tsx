...
function Downloads({ jobs, libraries, halt, refresh, onError, onReveal }: { jobs: DownloadJob[]; libraries: LibraryView[]; halt: QueueHalt | null; refresh: () => Promise<void>; onError: (e: unknown) => void; onReveal: (target: string) => void }) {
  ...
  const filtered = jobs.filter((job) => {
    ...
    return (!status || job.status === status) && `${job.title} ${job.target} ${destination(job.target).library ?? ""}`.toLocaleLowerCase(localeTag()).includes(query.trim().toLocaleLowerCase(localeTag())) && (!from || !!day && day >= from) && (!to || !!day && day <= to);
  }).sort((a, b) => {
